...
most_popular_breed <- most_popular_breed %>% filter(n == max(n, na.rm = TRUE)) %>% pull(BreedName)

proportion_top_20 <- dog_data %>% group_by(BreedName) %>% 
  summarise(n = n())
proportion_top_20 <- most_popular_breed[most_popular_breed$BreedName != "Unknown", ]
total_dogs <- proportion_top_20 %>% summarise(sum(n))
proportion_top_20 <- proportion_top_20 %>% top_n(20) %>% summarise(sum(n))
